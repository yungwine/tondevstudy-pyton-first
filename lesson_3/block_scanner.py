...
async def handle_block(block: BlockIdExt):
    ...
    for tr in trs:
        # op_code = tr.in_msg.body.begin_parse().load_uint(32)
        # if op_code == '0x11': ...
        if tr.in_msg.init:
            addr = Address((0, tr.account_addr))
            print(addr)
            try:
                await client.run_get_method(addr, method='get_wallet_data', stack=[])
                print('deployed jetton wallet')
            except RunGetMethodError as e:
                pass
            try:
                await client.run_get_method(addr, method='get_jetton_data', stack=[])
                print('deployed jetton minter')
            except RunGetMethodError as e:
                pass
            try:
                await client.run_get_method(addr, method='get_nft_data', stack=[])
                print('deployed nft')
            except RunGetMethodError as e:
                pass
